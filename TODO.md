...
> 最后更新：2026-09-25：游戏平面 P0 收官——消息撤回（窗口/频道可配 + 六态回码 + 墓碑广播 + 离线副本回收 + SDK `RecallMessage`），并记账撤回墓碑贯通历史存档的剩余缺口；覆盖率保持 100.0%。
>
...
- [ ] **撤回墓碑贯通历史存档**（撤回批次的剩余缺口，见 game_chat_features 备注：`GET_HISTORY` 仍返回原文。落地需要 store 面的 tombstone：`MessageStore` 加撤回标记 + MySQL 列/Hybrid Redis 镜像 + 历史返回体带 `is_recalled`；离线队列那一半已在撤回批次里单独补齐。横跨 3 个存储实现 + 一处 proto 字段，先按 P0 缺口记账，等真实合规诉求触发）
