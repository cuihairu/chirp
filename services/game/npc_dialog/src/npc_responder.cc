...
void NpcResponder::OnEvent(const chirp::game_server_gateway::EventDeliverNotify& event) {
  ...
  chirp::game_server_gateway::MessageInjectRequest reply;
  reply.set_inject_id(event.event_id());  // caller idempotency key = event id
  reply.set_sender_kind(chirp::game_server_gateway::SENDER_NPC);
...
void NpcResponder::Ack(const std::string& event_id) {
  chirp::game_server_gateway::EventAckRequest req;
