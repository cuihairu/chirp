...
InjectOutcome InjectConsumer::HandleInject(
    const chirp::server_gateway::InjectMessageNotify& notify) {
  const chirp::server_gateway::MessageInjectRequest& req = notify.message();
  ...
  if (req.content().empty() || req.sender_id().empty() ||
      req.sender_kind() == chirp::server_gateway::SENDER_UNKNOWN ||
