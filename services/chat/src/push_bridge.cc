...
PushBridge::PushBridge(std::shared_ptr<notification::NotificationClient> client)
...
void PushBridge::NotifyOffline(const ChatMessage& msg, const std::string& offline_user_id) {
  ...
  notification::PushNotificationRequest req;
  req.set_user_id(offline_user_id);
  req.set_type(notification::MESSAGE);
  ...
  client_->AsyncPush(req, /*seq=*/0, [](const notification::PushNotificationResponse& resp) {
