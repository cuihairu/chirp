...
TEST_F(ServerGatewayTest, InjectRejectsGameTaggedFanout) {
  // Game-tagged injections moved to the app_chat player directory (the spoke
  // uplink fans channel messages out there); this hub keeps injections +
  // events only and refuses the game_id shape.
  const auto chat = AuthAs("chat");
  auto req = ValidInject();
