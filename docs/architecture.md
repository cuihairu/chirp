...
2. **Subscription + fan-in routing.** The app needs a subscription registry (`player → {game, channel}`) and a routing path that fans per-game chat traffic into the player's aggregated feed, with unified unread counts across games. (2026-09: the registry (slice 2) and hub-side fan-in delivery — game-tagged injects fanned out to subscribers as private copies through chat's delivery tail — are live (slice 3); unified unread is the remaining piece.)
