...
| 聊天基础消息 | `chirp_chat`(`services/shared/chat/`) | Supported | 同一二进制按需部署为 `game_chat` 或 `app_chat`。私聊、历史、离线队列、群管理(建群/入群/退群/踢人/邀请/成员列表)、已读回执、正在输入提示、消息回应、消息编辑/删除(含管理员与批量删除)、@提及解析与自动补全、以及直连入口限流(每 IP 登录 / 每用户发送的固定窗口,Redis 支撑,故障放行)都接进了默认二进制。边缘过渡期直连客户端入口(7000/7001)保持开放;设置 `--gateway_service_secret` 后,通过 SERVER_AUTH_REQ 信任门的连接(网关管道)跳过每 IP 登录限流;设置 `--token_secret` 后,登录要求 HS256 JWT,密钥与 auth 的 `--jwt_secret` 对齐 |
